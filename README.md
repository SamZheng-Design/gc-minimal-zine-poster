...
`52–57` are a third series, **Plates**, pairing one-to-one with `46–51`: same six photographs, same six phrases, same six hues, but the photograph is printed as a full plate rather than reduced to an anchor. The reason is structural — the skill's `Image Anchor` vocabulary has exactly eight entries and every one of them shrinks, flattens or fragments the source image, so there is no way inside that vocabulary to say "show the photograph properly". This series therefore abandons the Color Engine's 0.8%–2.5% anchor-size rule outright; the plates measure 27.9%–41.4% of the sheet. It leans on two things the skill does say: *Prefer a colored … image panel*, and the fact that the negative constraint bans **full-bleed**, not scale. Everything else is kept — near-white stock, flat-scan view, minimal type, print artefacts, one hue per sheet.
